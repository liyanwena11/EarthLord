...
struct BackpackView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                HStack {
                    ...
                    Text(String(localized: "背包负重")).font(.headline)
                    Spacer()
                ...
                HStack(spacing: 12) {
                    Label("\(manager.backpackItems.count) " + String(localized: "种"), systemImage: "archivebox")
                    Label("\(totalQuantity) " + String(localized: "件"), systemImage: "number")
                    Spacer()
                    if manager.totalWeight >= manager.maxCapacity {
                        Text(String(localized: "已满载")).font(.caption).bold()
            ...
            if manager.backpackItems.isEmpty {
                VStack(spacing: 20) {
                    ...
                    Text(String(localized: "背包空空如也")).foregroundColor(.gray)
                    Text(String(localized: "探索废墟搜刮物资吧")).font(.caption).foregroundColor(.secondary)
                    Spacer()
                }
            } else {
                List {
        ...
        .navigationTitle(String(localized: "物资背包"))
        .onAppear {
        ...
        .overlay(alignment: .top) {
            if showUsedToast, let name = usedItemName {
                HStack(spacing: 8) {
                    ...
                    Text(String(localized: "已使用：") + name).font(.subheadline.bold()).foregroundColor(.white)
                }
                    ...
                    HStack(spacing: 12) {
                        ...
                        VStack(alignment: .leading, spacing: 3) {
                            ...
                            HStack(spacing: 6) {
                                ...
                                Text("\(String(format: "%.1f", item.weight))" + String(localized: "kg/个")).font(.caption2).foregroundColor(.secondary)
                                Text(String(localized: "共") + "\(String(format: "%.1f", item.totalWeight))kg").font(.caption2).foregroundColor(.secondary)
                            }
                        }
                    }
                }
            ...
            if showStory, let story = item.backstory {
                HStack(alignment: .top, spacing: 8) {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        Text(String(localized: "物品档案")).font(.system(size: 9, weight: .bold)).foregroundColor(.orange.opacity(0.7))
